...
def app():
    ...
    with app.app_context():
        ...
        # Create test users
        db = get_db()
        db.execute('INSERT INTO users (username, coin_balance) VALUES (?, ?)', ('alice', 5000))
        db.execute('INSERT INTO users (username, coin_balance) VALUES (?, ?)', ('bob', 3000))
        db.execute('INSERT INTO users (username, coin_balance) VALUES (?, ?)', ('charlie', 8000))
        db.commit()
...
def test_send_coins_success(authenticated_session, app):
    ...
    response = authenticated_session.post('/api/transfer',
                                        json={
                                            'sender': 'alice',
                                            'recipient': 'bob',
                                            'amount': 1000
    ...
    with app.app_context():
        db = get_db()
        alice_balance = db.execute('SELECT coin_balance FROM users WHERE username = ?', ('alice',)).fetchone()
        bob_balance = db.execute('SELECT coin_balance FROM users WHERE username = ?', ('bob',)).fetchone()
...
def test_recent_transactions_display(authenticated_session, app):
    ...
    authenticated_session.post('/api/transfer',
                             json={
                                 'sender': 'alice',
                                 'recipient': 'bob',
                                 'amount': 500
