...
def is_authenticated():
    ...
    timeout_threshold = datetime.now() - timedelta(seconds=current_app.config['SESSION_TIMEOUT_SECONDS'])
...
def require_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        if not is_authenticated():
            if request.is_json:
                return jsonify({'error': 'Authentication required', 'status': 'session_expired'}), 401
...
def register():
    ...
    return render_template('register.jinja2', title="Join The Revolution - Straw Coin Registration")
...
def login():
    ...
    if not data or 'username' not in data:
        return jsonify({'error': 'Username required', 'status': 'invalid_credentials'}), 400
...
def session_status():
    ...
    last_activity = datetime.fromisoformat(session['last_activity'])
    time_remaining = current_app.config['SESSION_TIMEOUT_SECONDS'] - (datetime.now() - last_activity).total_seconds()
...
def session_expired():
    return render_template('session_expired.jinja2', title="Session Expired - Straw Coin")
...
def track_activity():
    public_endpoints = ['auth.register', 'auth.session_expired', 'auth.login', 'auth.session_status', 
                       'auth.update_activity_endpoint', 'api.register_user']
    ...
    if 'username' in session:
        if not is_authenticated():
            if request.is_json:
                return jsonify({'error': 'Session expired', 'status': 'session_timeout'}), 401
