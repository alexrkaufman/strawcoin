...
class PerformerRedistributionScheduler:
    ...
    def start(self):
        ...
        if self.app:
            with self.app.app_context():
                current_app.logger.info("🎭 Started performer redistribution scheduler (60 second intervals)")
    ...
    def _perform_redistribution(self):
        ...
        try:
            from .db import performer_redistribution, cleanup_expired_sessions, is_market_open
            ...
            if result["success"]:
                ...
                current_app.logger.warning(f"⚠️ Redistribution failed: {result['message']}")
